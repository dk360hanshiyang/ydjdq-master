...
public class AES {
	...
	public static String encryptToBase64(String data, String key){
		try {
			...
			return new String(Base642.encode(valueByte));
	...
	public static String decryptFromBase64(String data, String key){
		try {
			byte[] originalData = Base642.decode(data.getBytes());
	...
	public static String encryptWithKeyBase64(String data, String key){
		try {
			byte[] valueByte = encrypt(data.getBytes(ConfigureEncryptAndDecrypt.CHAR_ENCODING), Base642.decode(key.getBytes()));
			return new String(Base642.encode(valueByte));
		} catch (UnsupportedEncodingException e) {
			throw new RuntimeException("encrypt fail!", e);
		}
	...
	public static String decryptWithKeyBase64(String data, String key){
		try {
			byte[] originalData = Base642.decode(data.getBytes());
			byte[] valueByte = decrypt(originalData, Base642.decode(key.getBytes()));
			return new String(valueByte, ConfigureEncryptAndDecrypt.CHAR_ENCODING);
		} catch (UnsupportedEncodingException e) {
			throw new RuntimeException("decrypt fail!", e);
		}
	...
	public static String genarateRandomKeyWithBase64(){
		return new String(Base642.encode(genarateRandomKey()));
	}
